...
public class SuffixArrayX {
    ...
    public int lcp(){
      ...
      for(int i=0; i<N; i++){
        if(rank[i]>0){
          int k = index[rank[i]-1];
          while(text[i+lcp]==text[k+lcp])
            lcp++;
        }
        lcpa[rank[i]]=lcp;
        sum += lcp;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; check && i < s.length(); i++) {
            if (suffixReference.index(i) != suffix.index(i)) {
                System.out.println("suffixReference(" + i + ") = " + suffixReference.index(i));
                System.out.println("suffix(" + i + ") = " + suffix.index(i));
        ...
        for (int i = 0; i < s.length(); i++){
            ...
            if (i == 0) {
              ...
              distinct += suffix.index(i)+1;
            }else {
              distinct += suffix.index(i)-suffix.lcp(i)+1;
              // int lcp  = suffix.lcp(suffix.index(i), suffix.index(i-1));
                int lcp  = suffix.lcp(i);
                System.out.printf("%3d %3d %3d %3d  %s\n", i, index, lcp, rank, ith);
            }
        }
        System.out.println("LCP sum = "+suffix.lcp());
        
        System.out.println("Total Distinct Subtstrings are "+distinct);
    }
